...
- `test/test_game.py` for integration-style checks
...
- `uv run python test/test_game.py` runs the current test script (world, database, character flows)
...
- Add scenario tests to `test/test_game.py` as `test_*` functions
...
Run tests with:
- `uv run python test/test_game.py`
